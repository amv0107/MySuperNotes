...
class SelectFormatDateTimeDialog : DialogFragment() {

    private val currentFormat: Int
        get() = requireArguments().getInt(ARG_VALUE)

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {

        val listFormat = DataStoreFormatDateTime.values().map {
            it.title
        }.toTypedArray()

        return AlertDialog.Builder(requireContext(), R.style.ConfirmDialogFragment)
            .setTitle(R.string.theme_confirm_dialog_date_title)
            .setSingleChoiceItems(listFormat, currentFormat, null)
    ...
    companion object {
        @JvmStatic
        private val TAG = SelectFormatDateTimeDialog::class.java.simpleName
        ...
        fun show(manager: FragmentManager, currentFormat: Int) {
            val dialogFragment = SelectFormatDateTimeDialog()
            dialogFragment.arguments = bundleOf(ARG_VALUE to currentFormat)
